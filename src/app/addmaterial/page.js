...
export default function MaterialsPage() {
  ...
  const createMaterial = async () => {
    ...
    if (
      ...
      (type === "document" && documentFiles.length === 0) ||
      (type === "presentation" && presentationFiles.length === 0) ||
      ((type === "test" || type === "opros" || type === "youtube") && testFilePath.trim() === "")
    ) {
    ...
    try {
      ...
      const materialResponse = await axios.post(
        `${host}/api/materials`,
        {
          title,
          type,
          file_path: type === "test" || type === "opros" || type === "youtube" ? testFilePath : uploadedFileResponse?.data.newFile.path,
          lesson_id: Number(lesson_id),
        },
        {
          headers: {
          ...
          <Box mt={4}>
            ...
            <FormControl fullWidth sx={{ mt: 2 }}>
              ...
              <Select value={type} onChange={(e) => setType(e.target.value)} fullWidth required>
                ...
                <MenuItem value="test">Ссылка на тест</MenuItem>
                <MenuItem value="opros">Ссылка на опрос</MenuItem>
                <MenuItem value="youtube">Ссылка на видео YouTube</MenuItem>
              </Select>
            </FormControl>
            ...
            {(type === "test" || type === "opros" || type === "youtube") && (
              <TextField
                label={
                  type === "test" ? "Ссылка на тест" : 
                  type === "opros" ? "Ссылка на опрос" : 
                    ...
                    <IconButton
                      ...
                      onClick={() => {
                        ...
                        if (type === "test" || type === "opros" || type === "youtube") {
                          setTestFilePath(material.file_path);
                        }
                      }}
                    >
                      <Edit />
